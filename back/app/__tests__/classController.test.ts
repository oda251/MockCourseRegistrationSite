...
describe("ClassControllerImpl", () => {
  ...
  describe("fetchRegistratedClasses", () => {
    it("should return registered classes for a user", async () => {
      ...
      const result = await classController.fetchRegistratedClasses(
        req,
        res,
        next,
      );
  ...
  describe("registerClass", () => {
    ...
    it("should register a class successfully", async () => {
      ...
      const req = { userId, body: { classId } } as unknown as Request<
        ParamsDictionary,
        string,
        RegisterParams
      >;
    ...
    it("should return 401 for double booking", async () => {
      ...
      expect(res.send).toHaveBeenCalledWith(
        "another class is already registered on the same time",
      );
    });

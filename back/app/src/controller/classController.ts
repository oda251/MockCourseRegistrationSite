		...
		try {
			...
			if (classes) {
				const isOverlap = classes.some((c) => {
					if (c.day === classWanted.day && c.period === classWanted.period && c.semester === classWanted.semester) {
						return true;
					}
					return false;
				});
				if (isOverlap) {
					response.status(StatusCodes.BAD_REQUEST).send("another class is already registered on the same time");
					return false;
				}
			}
